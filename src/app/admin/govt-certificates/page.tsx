...
import { Plus, Trash2, Edit2, Loader2, Save, X, Image as ImageIcon, Info } from 'lucide-react';
...
/** Read an image file and return its natural width/height. */
...
export default function AdminGovtCertificates() {
  ...
  // Enforced dimensions derived from the first cert image
  const [standardDims, setStandardDims] = useState<{ width: number; height: number } | null>(null);

  useEffect(() => {
    loadData();
  }, []);

  // When the list changes, detect standard dimensions from the first image
  const detectStandardDimensions = useCallback(async (items: GovtCertificate[]) => {
    const firstWithImage = items.find(i => i.image_url);
    if (!firstWithImage?.image_url) {
      setStandardDims(null);
      return;
    }
    try {
      const img = new window.Image();
      img.crossOrigin = 'anonymous';
      img.onload = () => setStandardDims({ width: img.naturalWidth, height: img.naturalHeight });
      img.onerror = () => setStandardDims(null);
      img.src = firstWithImage.image_url;
    } catch {
      setStandardDims(null);
    }
  }, []);
  ...
  async function loadData() {
    ...
    if (res.success && res.data) {
      setList(res.data);
      detectStandardDimensions(res.data);
    }
    setLoading(false);
  }
  ...
  const handleSaveClick = async () => {
    ...
    try {
      ...
      if (selectedFile) {
        ...
        // If standard dimensions exist, reject if the image doesn't match
        if (standardDims) {
          const dims = await getImageDimensions(selectedFile);
          if (dims.width !== standardDims.width || dims.height !== standardDims.height) {
            toast.error(`Image dimensions must be exactly ${standardDims.width}x${standardDims.height}`);
        ...
        const uploadRes = await uploadFileAction('govt_cert_images', fileToUpload.name, formData);
        setUploadingImage(false);
    ...
    <div className="space-y-8 max-w-[1000px] mx-auto">
      ...
      {!isFormOpen ? (
        <>
          ...
          {/* Dimension Standard Info Banner */}
          {standardDims && (
            <div className="flex items-start gap-3 bg-blue-50 border border-blue-200 rounded-xl p-4">
              <Info className="w-5 h-5 text-blue-600 mt-0.5 shrink-0" />
              <div>
                <div className="text-sm font-bold text-blue-900">Enforced Image Dimensions</div>
                <div className="text-xs text-blue-700 mt-0.5">
                  All certificate images must be exactly <strong>{standardDims.width}x{standardDims.height}</strong> (set by the first uploaded image).
                </div>
              </div>
            </div>
          )}
        ...
        <>
          ...
          {/* Dimension notice in the form */}
          {standardDims && (
            <div className="flex items-start gap-3 bg-amber-50 border border-amber-200 rounded-xl p-4 mb-2">
              <Info className="w-5 h-5 text-amber-600 mt-0.5 shrink-0" />
              <div>
                <div className="text-sm font-bold text-amber-900">Image Dimension Requirement</div>
                <div className="text-xs text-amber-700 mt-0.5">
                  Images must be exactly <strong>{standardDims.width}x{standardDims.height}</strong>.
                </div>
              </div>
            </div>
          )}
                    ...
                    <div className="border-2 border-dashed border-slate-300 rounded-xl p-6 flex flex-col items-center justify-center text-center bg-slate-50">
                      ...
                      {standardDims && (
                        <p className="text-[11px] text-slate-400 mt-3">
                          Required: {standardDims.width}x{standardDims.height}
                        </p>
                      )}
                    </div>
                  </div>
